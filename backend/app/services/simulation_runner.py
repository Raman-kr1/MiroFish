...
# Mark if cleanup function is registered
...
class SimulationRunState:
    ...
    # Platform completion status (Detect simulation_end event in actions.jsonl)
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Max simulation rounds (optional, for shortening long simulations)
        ...
        Args:
            ...
            max_rounds: Max simulation rounds (optional, for shortening long simulations)
            enable_graph_memory_update: Whether to update activities to Zep graph
        ...
        if not os.path.exists(config_path):
            raise ValueError(f"Simulation config not found, please call /prepare interface first")
        ...
        try:
            ...
            # Create main log file to prevent process blocking due to stdout/stderr pipe buffer fill
            ...
            # Set working directory to simulation directory (database etc. will be generated here)
            # Use start_new_session=True to create new process group, ensuring all subprocesses can be terminated via os.killpg
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group, ensure server can terminate all related processes on shutdown
            )
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If only one platform running, only check that platform
                                    # If running two platforms, both need to complete
                                ...
                                # Update round info (from round_end event)
                                    ...
                                    # Total rounds take max value of both platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Total time take max value of both platforms
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        """
        Cross-platform terminate process and its subprocesses
        ...
        if IS_WINDOWS:
            ...
            # /F = Force terminate, /T = Terminate process tree (including subprocesses)
            ...
            # Since start_new_session=True used, process group ID equals main process PID
    ...
    def _read_actions_from_file(
        ...
        Args:
            ...
            default_platform: Default platform (used when 'platform' field is missing in record)
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer platform in record, otherwise use default_platform
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation logs (used for forcing restart of simulation)
        
        Clean up simulation run logs (used for forcing a restart of simulation)
        ...
        - twitter_simulation.db (Simulation DB)
        - reddit_simulation.db (Simulation DB)
        ...
        files_to_delete = [
            ...
            "stdout.log",
            "stderr.log",
            "twitter_simulation.db",  # Twitter Platform DB
            "reddit_simulation.db",   # Reddit Platform DB
        ...
        # Clean run state in memory
        if simulation_id in cls._run_states:
            del cls._run_states[simulation_id]
        
        logger.info(f"Simulation logs cleanup completed: {simulation_id}, deleted files: {cleaned_files}")
    ...
    def cleanup_all_simulations(cls):
        ...
        Called when server shuts down to ensure all subprocesses are terminated
        """
            ...
            try:
                if process.poll() is None:  # Process still running
                    ...
                    try:
                        ...
                        # Process might not exist anymore, try direct terminate
                        try:
                            process.terminate()
                    ...
                    # Update state.json as well, set status to stopped
                    try:
                        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def register_cleanup(cls):
        ...
        # If not in debug mode, this env var won't exist, need to register as well
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Returns:
            Status detail dict, includes status, twitter_available, reddit_available, timestamp
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: Interview on both platforms simultaneously in dual-platform simulation, return integrated result
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by platform in each interview item)
                ...
                - None: Interview each Agent on both platforms simultaneously in dual-platform simulation
            timeout: Timeout (seconds)
    ...
    def close_simulation_env(
        ...
        Send close environment command to simulation, making it exit gracefully to wait-for-command mode
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Environment close command sent (Wait response timeout, environment might be closing)"
            }
    ...
    def get_interview_history(
        ...
        if platform in ("reddit", "twitter"):
            ...
            # If no platform specified, query both
