...
// Get maxRounds from query params during init
const maxRounds = ref(route.query.maxRounds ? parseInt(route.query.maxRounds) : null)
const minutesPerRound = ref(30) // Default 30 mins per round
...
const handleGoBack = async () => {
  ...
  addLog('Closing simulation to return to Step 2...')
...
const handleNextStep = () => {
  // Step3Simulation will handle report gen and routing
  // Backup method
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        if (configRes.success && configRes.data?.time_config?.minutes_per_round) {
          ...
          addLog(`Time config: Per round ${minutesPerRound.value} mins`)
        }
      } catch (configErr) {
...
const loadGraph = async (graphId) => {
  // Avoid flashing loading when simulating
  // Show loading on manual or initial
  if (!isSimulating.value) {
