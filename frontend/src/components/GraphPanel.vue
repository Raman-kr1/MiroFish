    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content still processing, please refresh manually later</span>
...
const expandedSelfLoops = ref(new Set()) // Expanded self-loops items
const showSimulationFinishedHint = ref(false) // Hint after simulation finishes
...
// Watch isSimulating to detect finish
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transition from simulating to not simulating, show hint
...
const renderGraph = () => {
  ...
  // Process edges, count edges between node pairs
  const edgePairCount = {}
  ...
  // Track index of edge between node pairs
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Self-loop edge - add one merged loop per node
    ...
    if (totalCount > 1) {
      ...
      // Increase curvature range with edge count
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // Flip curvature if edge direction is reversed
      // Ensures all edges share a reference frame to avoid overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulation - Adjust node distance based on edge count
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Adjust distance based on edge count between pair
    ...
    // Add gravity to center to cluster independent node groups
  ...
  const getLinkPath = (d) => {
    ...
    // Check for self-loop
    if (d.isSelfLoop) {
      // Self-loop: Draw arc leaving and returning to node
      const loopRadius = 30
    ...
    // Calculate control points - dynamic adjustment based on edge count/distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Perpendicular offset based on distance ratio for visibility
  ...
  const getLinkMidpoint = (d) => {
    ...
    // Check for self-loop
    if (d.isSelfLoop) {
      // Self-loop label pos: Right of node
      return { x: sx + 70, y: sy }
    }
  ...
  // Link labels background (white for clarity)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Record pos only, don't restart (distinguish click vs drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Check if true drag (movement > threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Restart simulation only on first true drag
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only cool down simulation if actually dragged
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // Update label pos (no rotation, horizontal for clarity)
