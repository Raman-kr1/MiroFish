<template>
  ...
  >
    <!-- Background Decoration: Tech grid lines (only visible when there are projects) -->
    ...
    <div class="section-header">
      ...
      <span class="section-title">Deduction History</span>
      <div class="section-line"></div>
    </div>

    <!-- Cards Container (only visible when there are projects) -->
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- Card Title (Use first 20 chars of simulation requirement as title) -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Start Simulation" and Step5 "Deep Interaction" must be started during runtime, historical playback is not supported</span>
            </div>
          </div>
        </div>
      </Transition>
...
let pendingState = null  // Record target state to execute
...
const containerStyle = computed(() => {
  ...
  // Expanded state: calculate height based on card count
  ...
  // Calculate actual required height: rows * card_height + (rows-1) * gap + small bottom padding
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Expand downwards, increase gap with title
    ...
    const x = offset * 35
    // Adjust start position, close to title but keep proper gap
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Clear previous debounce timer (new scroll intent overwrites old)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // If animating, only record state, handle after animation
        if (isAnimating) return
        
        // If target state is same as current, no processing needed
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Use debounce delay for state switch to prevent flickering
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if pending state still needs execution (might be overwritten by subsequent scroll)
          ...
          setTimeout(() => {
            ...
            // After animation end, check for new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay execution slightly to avoid too fast switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin, shrink viewport bottom upwards, requires more scroll to trigger expand
...
// Watch route change, reload data when returning to home
...
/* CSS pattern for fixed-spacing square grid */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Anchored top-left, expands downwards without shifting */
...
.cards-container {
  ...
  /* min-height calculated by JS based on card count */
}
