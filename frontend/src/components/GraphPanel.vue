...
const showEdgeLabels = ref(true) // comment
const expandedSelfLoops = ref(new Set()) // comment
const showSimulationFinishedHint = ref(false) // comment
const wasSimulating = ref(false) // comment
...
const closeDetailPanel = () => {
  ...
  expandedSelfLoops.value = new Set() // comment
}
...
const renderGraph = () => {
  ...
  // comment
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // comment
    ...
    if (isSelfLoop) {
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // comment
        }
      })
      return
    }
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // comment
      const x1 = sx + 8  // comment
      const y1 = sy - 4
      const x2 = sx + 8  // comment
      const y2 = sy + 4
      // comment
    ...
    const offsetRatio = 0.25 + pairTotal * 0.05 // comment
