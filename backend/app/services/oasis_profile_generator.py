...
2. Optimize prompt to generate very detailed persona
3. Distinguish individual entities and abstract group entities
"""
...
class OasisProfileGenerator:
    ...
    Convert entities in Zep graph to Agent Profile required by OASIS simulation
    
    Features:
    1. Call Zep graph retrieval function to get richer context
    2. Generate very detailed persona (including basic info, career history, personality traits, social media behavior, etc.)
    3. Distinguish individual entities and abstract group entities
    """
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep graph hybrid search function to get rich info related to entity
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        3. Rich info retrieved from Zep hybrid search
        """
        context_parts = []
        ...
        # 4. Use Zep hybrid search to get richer info
    ...
    def _is_individual_entity(self, entity_type: str) -> bool:
        """Judge if it is an individual type entity"""
        return entity_type.lower() in self.INDIVIDUAL_ENTITY_TYPES
    
    def _is_group_entity(self, entity_type: str) -> bool:
        """Judge if it is a group/institution type entity"""
    ...
    def _generate_profile_with_llm(
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    # No max_tokens set, let LLM run free
                )
        ...
        logger.warning(f"LLM persona generation failed ({max_attempts} attempts): {last_error}, using rule generation")
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # If JSON truncated, try closing it
        ...
        # Simple check: If no comma or closing bracket after last quote, string might be truncated
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # 1. First try to fix truncated situation
        content = self._fix_truncated_json(content)
        ...
        if json_match:
            ...
            try:
                ...
                # 5. If still failing, try more aggressive fixing
                try:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic persona for simulation, maximally restoring existing reality. Must return valid JSON format, all string values cannot contain unescaped newlines. Use English."
        return base_prompt
...
2. persona: Detailed persona description (2000 words plain text), must include:
   ...
   - Background (Important experiences, connection to event, social relationships)
   - Personality (MBTI type, core personality, emotional expression)
   - Social Media Behavior (Posting frequency, content preference, interaction style, language features)
   - Stance/Viewpoint (Attitude towards topics, content that might trigger anger/emotion)
   - Unique Features (Catchphrases, special experiences, hobbies)
   - Personal Memory (Important part of persona, introduce connection to event, and existing actions/reactions in event)
...
Important:
- All field values must be string or number, do not use newlines
...
1. bio: Official account bio, 200 words, professional
2. persona: Detailed account setting description (2000 words plain text), must include:
   - Institution Basic Info (Formal name, nature, establishment background, main function)
   ...
   - Content Features (Content type, posting frequency, active time)
   - Stance/Attitude (Official stance on core topics, handling of controversy)
   - Special Notes (Representative group profile, operation habits)
   - Institution Memory (Important part of persona, introduce connection to event, and existing actions/reactions in event)
3. age: Fixed 30 (Virtual age for institution account)
4. gender: Fixed "other" (Institution account use other)
...
8. interested_topics: Array of interested fields

Important:
- All field values must be string or number, no null values
- persona must be a coherent text description, do not use newlines
- Use English (except gender field must be "other")
- age must be integer 30, gender must be string "other"
- Institution account speaking must fit its positioning"""
    ...
    def _generate_profile_rule_based(
        ...
        if entity_type_lower in ["student", "alumni"]:
            ...
            return {
                ...
                "gender": "other",  # Institution use other
    ...
    def generate_profiles_from_entities(
        ...
        """
        Batch generate Agent Profile from entities (support parallel generation)
        ...
        Args:
            ...
            realtime_output_path: Realtime output file path (if provided, write once per generation)
        ...
        # Helper function to save profiles to file in realtime
        ...
        print(f"\n{'='*60}")
        print(f"Starting Agent Profile Generation - Total {total} entities, Parallel: {parallel_count}")
        print(f"{'='*60}\n")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    # Write to file in real-time (even if fallback profile)
                    save_profiles_realtime()
        
        print(f"\n{'='*60}")
        print(f"Profile generation completed! Total {len([p for p in profiles if p])} Agents generated")
        print(f"{'='*60}\n")
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter Profile as CSV format (conforming to OASIS official requirements)
        ...
        - description: External display, short bio visible to other users
        """
        import csv
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Use format consistent with to_reddit_format(), ensuring OASIS can read it correctly.
        Must contain user_id field, which is key for OASIS agent_graph.get_agent() matching!
        ...
        logger.info(f"Saved {len(profiles)} Reddit Profiles to {file_path} (JSON format, containing user_id field)")
