      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <button v-if="isComplete" class="next-step-btn" @click="goToInteraction">
            <span>Start Deep Interaction</span>
...
const toggleRawResult = (timestamp, event) => {
  ...
  // Adjust scroll position after DOM update to keep button in place
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract interview selection reason
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (name) {
          ...
          // Reason continuation (exclude summary paragraph at end)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          // No platform tag (very old format), treat whole as answer
          interview.twitterAnswer = answerText
        }
      }
      
      // Extract key quotes (compatible with multiple formats)
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extract relevant facts - full extraction, no limit
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Sub-questions from Drift Query Analysis'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      // Chinese and English placeholders
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "问题X：" or "问题X:" or "Question X:" (Backend new format)
      // 2. "1. " or "\n1. " (Number + dot, legacy format compatible)
      let matches = []
      let match

      // Try "问题X：" or "Question X:" format first
      // Adjusted regex to match both Chinese and English
      const cnPattern = /(?:^|[\r\n]+)(?:问题|Question\s*)(\d+)[：:]\s*/g
      ...
      // If no number found or only one found, return matching whole
      if (matches.length <= 1) {
    ...
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers (non-placeholder) and content is different
            ...
            return h('div', { class: 'qa-pair', key: qIdx }, [
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  ...
                  // Expand/Collapse Button (not shown for placeholder)
...
const renderMarkdown = (content) => {
  ...
  // Remove the leading level 2 header (## xxx) because the section title is displayed externally
  ...
  // Handle newlines - empty lines become paragraph breaks, single newlines become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Clean up <p><br> following block elements (caused by extra empty lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean up consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean up <br> before paragraph start tags following block elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-consecutive ordered list numbering: keep numbering incremental when single item <ol> is separated by paragraph content
