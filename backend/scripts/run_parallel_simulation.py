...
- Environment remains active after simulation completion (wait-for-command mode)
...
Log Structure:
    ...
    └── run_state.json       # Run state (for API query)
"""

# ============================================================
# Solve Windows Encoding Issue: Set UTF-8 encoding before any imports
...
if sys.platform == 'win32':
    ...
    # Force default encoding (affects open() function default encoding)
    # Note: This needs to be set at Python startup, runtime setting might not work
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai warnings about max_tokens (we intentionally leave it unset to let model decide)"""
...
# Add filter immediately upon module load, ensuring it takes effect before camel code execution
logging.getLogger().addFilter(MaxTokensWarningFilter())


def disable_oasis_logging():
    """
    Disable verbose logging from OASIS library
    OASIS logs are too redundant (recording every agent observation and action), we use our own action_logger
    """
...
# Twitter available actions (excluding INTERVIEW, INTERVIEW can only be triggered via ManualAction)
TWITTER_ACTIONS = [
...
# Reddit available actions (excluding INTERVIEW, INTERVIEW can only be triggered via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/Unspecified: Interview on both platforms simultaneously, return integrated result
        ...
        # Unspecified platform: interview both (if available)
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Default platform (can be overridden by platform in each interview item)
                - "twitter": Only interview on Twitter platform
                - "reddit": Only interview on Reddit platform
...
# Filter out non-core action types (these actions have low value for analysis)
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from the database and supplement with complete context information
    
    Args:
        db_path: Database file path
        last_rowid: The maximum rowid value read last time (using rowid instead of created_at because created_at formats differ across platforms)
    ...
    try:
        ...
        # This avoids issues with created_at format differences (Twitter uses integer, Reddit uses datetime string)
        cursor.execute("""
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Supplement context information (post content, user name, etc.)
...
def _enrich_action_context(
    ...
    """
    Enrich action context (post content, user name, etc.)
    ...
    try:
        ...
        # Create comment: add post information being commented on
...
async def run_twitter_simulation(
    ...
    # Get mapping of real Agent names from configuration (use entity_name instead of default Agent_X)
    ...
    total_actions = 0
    last_rowid = 0  # Track the last processed row number in database (use rowid to avoid created_at format differences)
    ...
    for round_num in range(total_rounds):
        ...
        # Always record round start regardless of active agents
        if action_logger:
        ...
        # Fetch actually executed actions from database and log
    ...
    # Note: Do not close environment, keep for Interview usage
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM configuration (if available, otherwise fallback to general config)
...
async def main():
    ...
    # Create shutdown event at the beginning of main function to ensure the entire program can respond to exit signals
    ...
    if wait_for_commands:
        ...
        # Command waiting loop (use global _shutdown_event)
        try:
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation scenario: do not exit after simulation completion, wait for interview command
    ...
    def signal_handler(signum, frame):
        ...
        # Do not use sys.exit() directly, let asyncio loop exit normally and clean up resources
