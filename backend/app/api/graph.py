...
def reset_project(project_id: str):
    """
    Reset project state (for re-building graph)
    """
    project = ProjectManager.get_project(project_id)
    ...
    # Reset to status ONTOLOGY_GENERATED
    if project.ontology:
...
def generate_ontology():
    ...
    try:
        ...
        try:
            ...
            return jsonify({
                "success": False,
                "error": f"Failed to generate ontology: {str(e)}"
            }), 500
...
def build_graph():
    ...
    try:
        ...
        if errors:
            ...
            return jsonify({
                "success": False,
                "error": "Config errors: " + "; ".join(errors)
            }), 500
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is already building, please do not resubmit. To force rebuild, add force: true",
