...
Features:
- Does not close the environment immediately after simulation completion, enters command waiting mode
...
class TwitterSimulationRunner:
    ...
    def __init__(self, config_path: str, wait_for_commands: bool = True):
        ...
        Args:
            ...
            wait_for_commands: Whether to wait for commands after simulation ends (default True)
        """
    ...
    def _create_model(self):
        ...
        Consistently use configuration from .env file in project root (highest priority):
        - LLM_API_KEY: API Key
        - LLM_BASE_URL: API Base URL
        - LLM_MODEL_NAME: Model Name
        """
        # Prioritize reading from .env
    ...
    def _get_active_agents_for_round(
        ...
        # Calculate probability based on each Agent's configuration
        candidates = []
    ...
    async def run(self, max_rounds: int = None):
        ...
        print(f"  - Total simulation hours: {total_hours} hours")
        ...
        if self.wait_for_commands:
            print("\n" + "=" * 60)
            print("Entering command waiting mode - Environment keeps running")
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close environment immediately after simulation ends, do not enter command waiting mode'
    )
    ...
    # Initialize logging configuration (use fixed filename, cleanup old logs)
...
def setup_signal_handlers():
    """
    Setup signal handlers to ensure correct exit upon receiving SIGTERM/SIGINT
    Give the program a chance to cleanup resources (close database, environment, etc.)
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            ...
            # Force exit if signal received repeatedly
            print("Forcing exit...")
            sys.exit(1)
