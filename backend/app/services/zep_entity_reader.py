"""
Zep Entity Reader & Filter Service
Read nodes from Zep graph and filter out nodes matching predefined entity types
"""
...
class EntityNode:
    ...
    def get_entity_type(self) -> Optional[str]:
        """Get entity type (exclude default Entity labels)"""
...
class ZepEntityReader:
    ...
    2. Filter out nodes matching predefined entity types (Nodes whose Labels are not just Entity)
    ...
    def _call_with_retry(
        ...
        Args:
            ...
            max_retries: Maximum retries (default 3, i.e., max 3 attempts)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(
                        f"Zep {operation_name} failed attempt {attempt + 1}: {str(e)[:100]}, "
    ...
    def get_all_nodes(self, graph_id: str) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Got {len(nodes_data)} nodes in total")
        return nodes_data
    ...
    def get_all_edges(self, graph_id: str) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Got {len(edges_data)} edges in total")
        return edges_data
    ...
    def filter_defined_entities(
        ...
        Filtering logic:
        - If node labels only contain "Entity", skip (this entity does not match our predefined types)
        - If node labels contain labels other than "Entity" and "Node", keep (matches predefined types)
        ...
        # Build map of node UUID to node data
    ...
    def get_entity_with_context(
        ...
        """
        Get single entity and its full context (edges and related nodes, with retry mechanism)
        ...
        try:
            ...
            # Get node edges
            edges = self.get_node_edges(node_uuid=entity_uuid)
