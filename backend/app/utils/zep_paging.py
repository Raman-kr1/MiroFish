...
This module encapsulates automatic pagination logic (including single-page retry), returning the complete list transparently to the caller.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Single page request, with exponential backoff on failure. Retries only network/IO transient errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Get graph nodes with pagination, returning up to max_items (default 2000). Each page request has built-in retry."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Get all graph edges with pagination, returning the complete list. Each page request has built-in retry."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
