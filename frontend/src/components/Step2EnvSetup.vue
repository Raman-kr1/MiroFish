    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Create new simulation instance, pull simulation world parameter templates
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combine context, automatically call tools to sort out entities and relationships from the knowledge graph, initialize simulation individuals, and endow them with unique behaviors and memories based on reality seeds.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently sets world time flow speed, recommendation algorithms, active time periods for each individual, posting frequency, event triggering, etc., based on simulation requirements and reality seeds.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Automatically generate initial activation events and hot topics based on narrative direction to guide the initial state of the simulation world.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <p class="description">Simulation environment prepared, ready to start simulation.</p>
          
          <!-- Round Config - only show when config is generated and rounds are calculated -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Rounds Setting</span>
                <span class="section-desc">MiroFish automatically plans to deduce reality for <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours, with each round representing <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of time lapse.</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>If 100 Agents: Estimated time approx. {{ Math.round(customMaxRounds * 0.6) }} minutes</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        If 100 Agents: Estimated time approx. {{ Math.round(autoGeneratedRounds * 0.6) }} minutes
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For the first run, it is strongly recommended to switch to 'Custom Mode' to reduce simulation rounds for quick preview and lower error risk ➝</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              <span class="info-label">Event Explicit Age</span>
            ...
            <div class="info-item">
              <span class="info-label">Event Explicit MBTI</span>
...
const useCustomRounds = ref(false) // default use auto config rounds
...
watch(currentStage, (newStage) => {
  if (newStage === '生成Agent人设' || newStage === 'generating_profiles') {
    ...
    if (!configTimer) {
      addLog('Starting to generate dual-platform simulation configuration...')
      startConfigPolling()
    }
...
const autoGeneratedRounds = computed(() => {
  ...
  // Ensure max rounds not less than 40 (recommended), avoid slider range issue
...
// Sum of related topics for all personas
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    // User chose to keep auto configuration, do not pass max_rounds
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Detected existing completed work, using directly')
      ...
      // Immediately set expected total agents (from prepare interface return)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.progress_detail) {
        ...
        // Output detailed progress log (avoid dupes)
        ...
        // Output message log (avoid dupes)
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Only update when API returns valid value, avoid overwriting
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Output config generation stage log (avoid dupes)
      ...
      if (data.config_generated && data.config) {
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('✓ Environment setup completed, ready to simulate')
...
const loadPreparedData = async () => {
  ...
  // Get config (use realtime interface)
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog('Configuration generating, starting poll/wait...')
        startConfigPolling()
      }
    }
  } catch (err) {
