...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        ...
        # Directly return "Agent Name: Activity Description" format, without adding simulation prefixes
    ...
    def _describe_quote_post(self) -> str:
        """Quote post - includes original content, author info, and the quote comment"""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")
    ...
    def _describe_follow(self) -> str:
        """Follow user - includes the target user's name"""
    ...
    def _describe_create_comment(self) -> str:
        """Create comment - includes comment content and the replied post info"""
        content = self.action_args.get("content", "")
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_mute(self) -> str:
        """Mute user - includes the target user's name"""
        target_user_name = self.action_args.get("target_user_name", "")
...
class ZepGraphMemoryUpdater:
    ...
    All meaningful actions are updated to Zep, and action_args effectively contains full context information:
    - Original post content for Like/Dislike
    - Original post content for Repost/Quote
    - Usernames for Follow/Mute
    - Original comment content for Like/Dislike Comment
    """
    
    # Batch sending size (send after accumulating this many activities per platform)
    BATCH_SIZE = 5
    
    # Platform display names (for console display)
    PLATFORM_DISPLAY_NAMES = {
        'twitter': 'World1',
        'reddit': 'World2',
    }
    
    # Send interval (seconds) to avoid too many requests
    SEND_INTERVAL = 0.5
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # Platform-grouped activity buffers (accumulate up to BATCH_SIZE separately for each platform)
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args will contain full context information (e.g., original post content, usernames, etc.).
    ...
    def _flush_remaining(self):
        ...
        # First process activities remaining in the queue, adding them to buffers
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "items_sent": self._total_items_sent,        # Items successfully sent
