...
This script reads parameters from the configuration file to run the simulation, enabling full automation.
...
class IPCHandler:
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict]) -> bool:
        ...
        try:
            ...
            actions = {}
            agent_prompts = {}  # Track the prompt for each agent
...
class TwitterSimulationRunner:
    ...
    def _create_model(self):
        ...
        # Fall back to config if not found in .env
        if not llm_model:
        ...
        if not os.environ.get("OPENAI_API_KEY"):
            raise ValueError(
                "Missing API Key configuration, please set LLM_API_KEY in the .env file at project root"
            )
        ...
        print(
            ...
            f"base_url={llm_base_url[:40] if llm_base_url else 'not set'}..."
        )
    ...
    def _get_active_agents_for_round(
        ...
        """
        Determine which Agents to activate for this round based on the time and configuration.
...
async def main():
    parser = argparse.ArgumentParser(description='OASIS Twittertranslated')
    parser.add_argument(
        '--config', 
        type=str, 
        required=True,
