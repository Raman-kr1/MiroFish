...
# Adding this prefix can prevent Agent from calling tools, replying with text directly
...
def get_graph_entities(graph_id: str):
    ...
    Only returns nodes matching defined entity types (nodes where Labels are not just Entity)
...
def create_simulation():
    ...
    Note: Parameters like max_rounds are generated by LLM, no need to set manually
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Project graph not built, please call /api/graph/build first"
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Running scripts (run_*.py) are kept in backend/scripts/ directory, not copied to simulation directory
    ...
    try:
        ...
        # Detailed log
        logger.debug(f"Checking simulation ready status: {simulation_id}, status={status}, config_generated={config_generated}")
        
        # If config_generated=True and files exist, consider prepared
        # Following statuses indicate preparation is done:
        # - ready: Preparation completed, ready to run
        # - preparing: If config_generated=True, means completed
        # - running: Running, means preparation was done
        # - completed: Run completed, means preparation was done
        # - stopped: Stopped, means preparation was done
        # - failed: Run failed (but preparation is done)
...
def prepare_simulation():
    ...
    - If prepared, returns existing result directly
    - Supports forced regeneration (force_regenerate=true)
    
    Steps:
    1. Check if preparation is already done
    ...
    try:
        ...
        try:
            ...
            reader = ZepEntityReader()
            # Fast read entities (no edge info, just count)
            ...
            # Failure doesn't affect flow, background task will retry
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Iterate reports directory, find report matching simulation_id,
    if multiple found, return the latest one (sorted by created_at)
