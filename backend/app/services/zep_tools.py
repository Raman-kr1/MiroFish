...
class InsightForgeResult:
    """
    InsightForge Result
    Includes retrieval results for multiple sub-questions, and comprehensive analysis
    """
    query: str
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            f"## Future Prediction In-depth Analysis",
        ...
        if self.semantic_facts:
            text_parts.append(f"\n### [Key Facts] (Please cite these verbatim in report)")
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            f"## Panorama Search Result (Future Panoramic View)",
            f"Query: {self.query}",
            f"\n### Statistics",
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter garbage content containing question numbers (Question 1-9)
                skip = False
                for d in '123456789':
                    if f'Question {d}' in clean_quote or f'Question{d}' in clean_quote or f'\u95ee\u9898{d}' in clean_quote:
                        skip = True
                        break
                if skip:
                    continue
                ...
                if len(clean_quote) > 150:
                    dot_pos = -1
                    # Try to find english period first
                    p_pos = clean_quote.find('.', 80)
                    if p_pos > 0:
                        dot_pos = p_pos
                    else:
                        # Try chinese period
                        c_pos = clean_quote.find('\u3002', 80)
                        if c_pos > 0:
                            dot_pos = c_pos
...
class ZepToolsService:
    ...
    - get_all_edges - Get all edges in graph (including time info)
    ...
    def search_graph(
        ...
        Search for relevant information in the graph using hybrid search (Semantic + BM25).
        If Zep Cloud's search API is unavailable, degrades to local keyword matching.
        ...
        try:
            ...
            logger.warning(f"Zep Search API failed, degraded to local search: {str(e)}")
            # Degrade: Use local keyword matching search
    ...
    def _local_search(
        ...
        """
        Local Keyword Matching Search (As degradation plan for Zep Search API)
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Get all edges in graph (Paginated, includes time info)
        ...
        Returns:
            List of edges (Contains created_at, valid_at, invalid_at, expired_at)
        """
    ...
    def insight_forge(
        ...
        1. Use LLM to decompose question into multiple sub-questions
        ...
        Args:
            ...
            max_sub_queries: Max number of sub-questions
...
Requirements:
1. Each sub-problem should be specific enough to find relevant Agent behaviors or events in the simulated world
...
Return JSON formatted list of sub-questions."""
        ...
        try:
            ...
            # Degrade: Return variants based on original question
            return [
                query,
    ...
    def panorama_search(
        ...
        Get panoramic view, including all relevant content and historical/expired information:
        1. Get all relevant nodes
        2. Get all edges (including expired/invalid)
        3. Classify and organize current active and historical information
        ...
        # Get all edges (including time info)
    ...
    def interview_agents(
        ...
        Call real OASIS interview API to interview running Agents in simulation:
        1. Automatically read persona files to understand all simulated Agents
        2. Use LLM to analyze interview requirements and intelligently select most relevant Agents
        ...
        - Need to get real answers from simulated Agents (Non-LLM simulation)
        ...
        INTERVIEW_PROMPT_PREFIX = (
            "You are accepting an interview. Please combine your persona, all past memories and actions,"
            ...
            "4. Answer one by one by question number, start each answer with 'Question X:' (X is question number)\n"
            "5. Separate answers to each question with a blank line\n"
            "6. Answers should be substantive, answer at least 2-3 sentences for each question\n\n"
        )
        ...
        try:
            ...
            # Call SimulationRunner batch interview method (no platform passed, dual platform interview)
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Strategy 2 (Supplement): Long text within correctly paired quotes (English "" and Chinese “”「」)
                if not key_quotes:
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Clean JSON tool call wrapper in Agent response, extract actual content"""
    ...
    def _select_agents_for_interview(
        ...
        Returns:
            tuple: (selected_agents, selected_indices, reasoning)
                - selected_agents: List of complete info of selected Agents
                - selected_indices: List of indices of selected Agents (for API call)
        ...
        system_prompt = """You are a professional interview planning expert. Your task is to select the most suitable candidates for interview from the simulated Agent list based on interview requirements.

Selection Criteria:
1. Agent identity/profession is relevant to the interview topic
2. Agent may hold unique or valuable viewpoints
3. Select diverse perspectives (e.g., Supporter, Opponent, Neutral, Professional, etc.)
...
Please select up to {max_agents} most suitable Agents for interview and explain the reasons."""
        ...
        try:
            ...
            # Get complete info of selected Agents
            selected_agents = []
            valid_indices = []
    ...
    def _generate_interview_questions(
        ...
        system_prompt = """You are a professional reporter/interviewer. Generate 3-5 in-depth interview questions based on interview requirements.

Question Requirements:
1. Open-ended questions, encouraging detailed answers
2. Different answers may be expected for different roles
...
Please generate 3-5 interview questions."""
    ...
    def _generate_interview_summary(
        ...
        system_prompt = """You are a professional news editor. Please generate an interview summary based on the answers of multiple interviewees.

Summary Requirements:
1. Extract main viewpoints of all parties
...
Format Constraints (Must comply):
...
- Use quotes when citing interviewee's original words
