...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Passed from Step2
  minutesPerRound: {
    type: Number,
...
// Computed
// Display actions chronologically (latest at bottom)
...
// Platform specific elapsed time
...
// Track previous rounds to log changes
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Check individual platform completion status
...
const checkPlatformsCompleted = (data) => {
  ...
  // Determine if platforms are enabled (active actions or running flag)
